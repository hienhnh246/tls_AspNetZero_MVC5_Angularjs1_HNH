...
using triluatsoft.tls.MultiTenancy.Dto;
using triluatsoft.tls.Notifications;
using Shouldly;
using Xunit;

namespace triluatsoft.tls.Tests.MultiTenancy
{
        ...
        public async Task Create_Update_And_Delete_Tenant_Test()
        {
            ...
            await _tenantAppService.CreateTenant(
                new CreateTenantInput
                {
                    ...
                    IsActive = true,
                    ConnectionString = "Server=localhost; Database=tlsTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;"
                });
