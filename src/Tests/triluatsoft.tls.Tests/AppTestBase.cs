...
using triluatsoft.tls.Migrations.Seed.Host;
using triluatsoft.tls.Migrations.Seed.Tenants;
using triluatsoft.tls.MultiTenancy;
using triluatsoft.tls.Tests.TestDatas;
...
namespace triluatsoft.tls.Tests
{
    ...
    /// Provides methods to easily work with <see cref="tlsDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<tlsTestModule>
    {
        ...
        protected void UsingDbContext(Action<tlsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<tlsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<tlsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<tlsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<tlsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<tlsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<tlsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<tlsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<tlsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<tlsDbContext, Task<T>> func)
        {
            T result;
